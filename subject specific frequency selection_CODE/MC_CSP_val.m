%this version include the validation set. original MC_CSP contains only
%test and train set

function [Train_Mat,Test_Mat,Valid_Mat] = MC_CSP_val(train_data,test_data,data_val,nbFilterPairs)

train_data.x = permute(train_data.x,[2,1,3]);
test_data.x = permute(test_data.x,[2,1,3]); 
data_val.x = permute(data_val.x,[2,1,3]);
...
%     Filter = CSPMatrix([1:nbFilterPairs (end-nbFilterPairs+1):end],:);
   ...
   valid_trials = size(data_val.x,3); 
   for a=1:valid_trials
       Valid_Mat(:,:,a) = Filter * data_val.x(:,:,a)';
   end
