...
load feat_fus_log_var_win %feat_fus_log_var_win   feat_fus_log_var
...
for s = 1:size(feat_bank,3)%subjects
    ...
    while (length(FF)>0)
        ...
        for i=1:length(FF)
            res(i) = feat_fus_res_sub_specific([tFF FF(i)],feat_bank,classifier,s);  % fprintf(' %2.2f ',res(i)); 
...
fprintf('Increasing add algorithm AVERAGE = %2.2f \n', mean(resS));
fprintf('\n');
